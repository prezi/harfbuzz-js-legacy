...
export LDFLAGS="-pipe -Os -no-cpp-precomp -arch i386"
...
./configure --host i386 --prefix=$INSTALL_DIR --disable-shared --enable-static && make clean >/dev/null && make -j && cp src/.libs/libharfbuzz.a build/harfbuzz.a

echo "### compile wrapper"
gcc -arch i386  -I ./src -I $JNIHEADERS  -pipe -Os -no-cpp-precomp -shared -o swig/build/cpp/lib/harfbuzz_wrap.so swig/build/cpp/harfbuzz_wrap.c  build/harfbuzz.a
